...
(function ($) {
   ...
   // var _defaultHideButtons = { append: false, removeLast: false, insert: false, remove: false, moveUp: false, moveDown: false };
    var _methods = {
        init: function (options) {
            ...
            if (target.length > 0) {
                ...
                $(tbWhole).attr({'data-role': 'table','data-mode':'reflow'});
                ...
                if(isEmpty(settings.showSelectButton))
                {
                     settings.showSelectButton = false;
                }

                var template = jQuery.validator.format("<th 'data-priority'={0}>{1}</th>");
                thCell = $(template("1", "")).appendTo(tbHeadRow)
                if(!settings.showSelectButton)
                {
                    $(thCell).appendClass('invisible');
                }
                ...
                for (var z = 0; z < settings.columns.length; z++) {                    
                    ...
                    if (settings.columns[z].type != 'hidden') {
                        ...
                        if (settings.columns[z].displayCss) $(thCell).css(settings.columns[z].displayCss);
                        // Add tooltip
                ...
                //$('th.resizable', tbHead).resizable({ handles: 'e' }); 
                ...
                if(settings.showSelectButton) settings.finalColSpan ++;
    ...
    function createLabel(tbCell, label, ctrlId, ctrlName)
    {
        ...
        var ctrl = $(template(ctrlId, ctrlId, label)).appendTo(tbCell).css({'display':'inline'});                        
        return ctrl;         
    }
...
function createListview(tbCell, data)
{
 var listViewHeader = $("<div></div>").appendTo(tbCell);
  listViewHeader.addClass('my-page').attr({'data-role':'collapsibleset','data-iconpos':'right', 'data-content-theme':'a'});
  var listView =$('<div></div>').appendTo(listViewHeader);
  listView.attr({'data-role':'collapsible', 'data-collapsed':'true'});
  $('<h2>section 1</h2>').appendTo(listView);
  var ul = $('<ul></ul>').appendTo(listView);
      ul.attr({'data-role':'listview', 'data-inset':'true'});
      for(var i=0; i<6;i++)
  {
 ...
 return listViewHeader;
    ...
    function insertRow(tbWhole, numOfRowOrRowArray, rowIndex, callerUniqueIndex) {
        ...
        for (var z = 0; z < numOfRow; z++) {
            ...
            var selectId = settings.idPrefix + '_select_td_' + uniqueIndex;
            var tbCell = $("<td></td>").appendTo(tbRow);                  
            createCheckbox(tbCell,selectId, selectId, selectId, null);

            if(!settings.showSelectButton)
            {
               tbCell.className += 'invisible';
            }
